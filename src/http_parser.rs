...
fn normalize_url(url_str: &str) -> Option<String> {
    match Url::parse(url_str) {
        ...
        Err(e) => {
            // Use warn instead of debug for parsing failures as they might indicate evasion attempts
            warn!(url = %url_str, error = %e, "Failed to parse URL");
            None
        }
    }
}
...
fn normalize_relative_url(path: &str) -> Option<String> {
    ...
    match Url::parse(base) {
        Ok(base_url) => {
            let joined_result = base_url.join(path);
            match joined_result {
                Ok(url) => {
                    ...
                    let mut decoded_path = path_and_query;
                ...
                Err(e) => {
                    warn!(path = %path, error = %e, "Failed to join URL");
                    None
                }
            }
        }
        Err(e) => {
            warn!(error = %e, "Failed to create base URL for relative path normalization");
            None
        }
    }
}
